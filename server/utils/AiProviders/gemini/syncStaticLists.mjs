...
if (fs.existsSync(existingCachePath)) {
  console.log(
    'Removing existing cache so we can fetch fresh models from Gemini endpoints...'
  );
...
function updateDefaultModelsFile(models) {
  const stableModelKeys = models
    .filter(model => !model.experimental)
    .map(model => model.id);
  const v1BetaModelKeys = models
    .filter(model => model.experimental)
    .map(model => model.id);

  let defaultModelFileContents = fs.readFileSync(
    path.join('./defaultModels.js'),
    'utf8'
  );
  ...
  console.log(
    'Updated defaultModels.js. Dont forget to `yarn lint` and commit!'
  );
}
